...
for line in infile:
    line = line.split()
    gene = (line[0].strip(),)
    c.execute("SELECT desc FROM description INNER JOIN bactmembers ON (description.NOGmember == bactmembers.NOGmember) WHERE bactmembers.eggNOGid == (SELECT nog FROM lategenes2NOGs WHERE gene = ?);", gene)
    cat = c.fetchall()
    try:
