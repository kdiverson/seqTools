...
for line in infile:
    line = line.split()
    gene = (line[0].strip(),)
    c.execute("SELECT cat FROM funccat INNER JOIN bactmembers ON (funccat.NOGmember == bactmembers.NOGmember) WHERE bactmembers.eggNOGid == (SELECT nog FROM allFull2nog WHERE gene = ?);", gene)
    cat = c.fetchall()
    c.execute("SELECT taxid FROM gi2tax WHERE gid = (select substr(alias,4,10000000) from aliases where eggNOGid = (SELECT nog FROM allFull2nog WHERE gene = ?) AND alias LIKE 'GI:%');", gene)
    taxid = c.fetchall()
