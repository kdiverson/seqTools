...
outfile = open("funcatbytaxEARLY.txt", 'w')
...
for n in sys.argv[1:]:
    ...
    for line in infile:
        ...
        cat = c.fetchall()
        c.execute("SELECT taxid FROM gi2tax WHERE gid = (select substr(alias,4,10000000) from aliases where eggNOGid = (SELECT nog FROM earlygenes2NOGs WHERE gene = ?) AND alias LIKE 'GI:%');", gene)
        taxid = c.fetchall()
